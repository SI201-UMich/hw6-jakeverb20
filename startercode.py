...
def search_breed(breed_id):
   ...
   """
   response_url = f"https://dogapi.dog/api/v2/breeds/{breed_id}"
   try:
       ...
       if response.status_code == 200:
           ...
           if parsed_json_dict.get("data") is not None:
               return (parsed_json_dict, response_url)        
...
def update_cache(breed_ids, cache_file):
    ...
    new_successful_adds = 0
    
    for brd_id in breed_ids:
        url = f"https://dogapi.dog/api/v2/breeds/{brd_id}"
        
        if url in cache:
            continue
        
        result = search_breed(brd_id)
        
        if result:
           parsed_json, request_url = result
           cache[request_url] = parsed_json
           new_successful_adds += 1
          
    create_cache(cache, cache_file)
    total_requested = len(breed_ids)
  
    if total_requested == 0:
       percentage = 0.0
    else:
       percentage = (new_successful_adds / total_requested) * 100
...
def get_longest_lifespan_breed(cache_file):
   ...
   for url in cache:
       try:
           ...
           lifespan = int(breed_data['attributes']['life']['max'])
          
           if lifespan > max_lifespan_integer:
               max_lifespan_integer = lifespan
               breed_name = name
              
           elif lifespan == max_lifespan_integer:
               if breed_name is None or name < breed_name:
                   breed_name = name
   ...
   if breed_name:
       ...
       return (breed_name, max_lifespan_integer)
